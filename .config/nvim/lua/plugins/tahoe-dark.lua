...
_G.tahoe_dark_setup = setup

local cache_dir = vim.fn.stdpath("cache") .. "/tahoe-dark"
local colors_dir = cache_dir .. "/colors"
vim.fn.mkdir(colors_dir, "p")
local f = io.open(colors_dir .. "/tahoe-dark.lua", "w")
if f then
  f:write([[
vim.g.colors_name = "tahoe-dark"
if _G.tahoe_dark_setup then
  _G.tahoe_dark_setup()
else
  require("plugins.tahoe-dark")
  if _G.tahoe_dark_setup then
    _G.tahoe_dark_setup()
  end
end
