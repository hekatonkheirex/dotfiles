...
- Fullscreen detection reads `is_fullscreen` off the focused window per output; drives the same pill-retracts-and-mask-clears behavior Ukishima gets from `Hyprland.monitors[].activeWorkspace.lastIpcObject.hasfullscreen`.
