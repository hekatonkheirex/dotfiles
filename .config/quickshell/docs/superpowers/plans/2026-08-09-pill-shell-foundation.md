...
- ~~Reuse existing primitives via relative import (`../../bar/SliderControl.qml` etc.) rather than forking copies into `pill/components/`~~ **Superseded during Task 6** (verified live): Quickshell's `-p <dir>` launch mode restricts its module scanner to the launched config's own root folder. `config/` resolves fine across that boundary only because it has a `qmldir` registering its contents as singletons (a different resolution path); `bar/` has no `qmldir` at all, so a bare directory import of its plain component types (`SwitchControl`, `SliderControl`, `PopupDivider`) is rejected with `"is outside of the config folder"` / `"X is not a type"`. Giving `bar/` a `qmldir` would require declaring all ~40 of its files as a formal module (any file left off becomes invisible, even to same-directory siblings — the same failure mode Task 6 hit with `pill/qmldir`), an ongoing maintenance burden on the live production shell not justified for this plan. Decision: fork `SliderControl.qml`, `SwitchControl.qml`, `PopupDivider.qml` into `pill/components/` instead — zero changes to `bar/`, `pill/` stays fully self-contained. `AudioSurface.qml` now imports `"../components"` instead of `"../../bar"`.
