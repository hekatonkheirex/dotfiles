...
# Wallpaper application is independent of theme generation. Acquire the
# shared lock only for the palette/theme refresh below so a startup refresh
# cannot delay the visible wallpaper change.
...
# Colors.qml's FileView watchChanges does not reliably live-reload the fresh
# palette, so ask the running shell to reload it explicitly. Keep the service
# restart as a fallback for startup or IPC failures.
if [[ "$matugen_updated" == true && "$color_source" != "fixed" ]]; then
  if command -v qs >/dev/null 2>&1 && qs ipc call shell reloadPalette >/dev/null 2>&1; then
    :
  else
