...
- **Bar.qml**: Single component for all four placements and both display styles (continuous full bar / floating pills bar). In pills mode, each visible widget has its own floating surface. Widgets follow a saved Start/Middle/End order in either orientation: a flexible divider separates the edge groups, while the Middle group is anchored independently to the bar midpoint. Settings and power remain at the far edge. The clock defaults to its theme position (centered in most styles, right-side slot in horizontal Ghost) and can join any ordered region.
- **WorkspaceIndicator**: Streams workspaces from Niri (`niri msg event-stream`) using `SplitParser` and runs only when visible. Moves as one widget when reordered. Horizontal markers center within their allocated track, including Classic's numbered controls.
- **Mango layout indicator**: Reads the active tag's layout from the shared `mmsg watch all-monitors` stream. Defaults between Workspaces and Focused window, but can move independently under General > Bar contents.
...
- **Settings**: Provides twelve tabs, remembers the last selected tab, keeps Network and Bluetooth Settings-only, and preserves Settings/power entry points when bar content switches are disabled. The panel moves from its header and resizes from the bottom-right corner, with a 560px minimum width and a maximum height bounded by screen margins. Sidebar navigation scrolls on overflow without a visible scrollbar; keyboard navigation brings the selected tab into view. Search prioritizes control titles, reports result counts or an empty state, and opens and highlights matching controls.
- **Settings presentation**: Defaults to a 1100×900 panel when no saved size exists. The complete Settings surface follows DankMaterialShell's grouped-navigation and tonal-section grammar: a 280px Material rail with icon badges, a narrower labeled rail below 700px, a shared page-heading hierarchy, and section surfaces across all twelve tabs. Nothing, Ghost, and Liquid retain their own color and shape language. Existing settings and save behavior are unchanged.
- **Bar widget relocation**: In Settings → General → Bar contents, all widgets—including the clock—use the same up/down, middle, and visibility controls. Arrows reorder widgets across Start/Middle/End; the middle button places a widget at the midpoint or moves it back to End. Hidden widgets keep their saved positions. Until moved, the clock follows the selected theme: centered in Material and vertical bars, at the end in horizontal Ghost. Moving it makes its chosen region explicit across theme changes; Reset all restores automatic clock placement and the default order. Existing two-region saved layouts load with an empty Middle region.
