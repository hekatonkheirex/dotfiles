...
# Terminal theme (STARSHIP_CONFIG + FZF_DEFAULT_OPTS), regenerated by
# sync-terminal-theme.sh whenever light/dark mode or colorscheme changes.
zmodload zsh/stat 2>/dev/null
[ -f "$HOME/.config/theme-env.sh" ] && source "$HOME/.config/theme-env.sh"
_THEME_ENV_MTIME=$(zstat +mtime "$HOME/.config/theme-env.sh" 2>/dev/null)

# Re-source theme-env.sh whenever it changes, so an already-open shell
# picks up a light/dark toggle live. Checked before every prompt via mtime
# (cheap stat, no-op if unchanged) instead of a signal: a signal-based
# broadcast kills any zsh process that hasn't loaded this hook yet, since
# an untrapped signal's default action is to terminate the process.
_theme_env_reload() {
  local mtime=$(zstat +mtime "$HOME/.config/theme-env.sh" 2>/dev/null)
  if [[ -n "$mtime" && "$mtime" != "$_THEME_ENV_MTIME" ]]; then
    _THEME_ENV_MTIME=$mtime
